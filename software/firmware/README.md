...
ESP32 firmware for acquiring voltage and current signals, calculating electrical measurements and sending them to the web application. The program is in `src/main.cpp`; network credentials and the server URL belong in the local `include/secrets.h` file.
...
Set `SERVER_URL` to your HTTPS endpoint ending in `/app/api.php`. The Apache configuration also accepts the previous `/app/app.php` URL for existing devices. Set `DEVICE_API_KEY` to the same value as the web application's `DEVICE_API_KEY`. Use a random hexadecimal key: the current firmware concatenates form fields without URL encoding, so characters such as `&`, `+` and `=` must not be used in that key.
...
PlatformIO installs the Espressif platform on the first build. Measurements are sent directly to the PHP application using the ESP32 Wi-Fi and HTTP libraries. The platform version is not pinned in `platformio.ini`; record the version used for your build so you can reproduce it later.
...
The ADC offset filter removes the DC component. Calibration coefficients are defined in `src/main.cpp`. Check and adjust them against your measurement hardware and a suitable reference instrument before using the readings.
...
The HTTPS client calls `setInsecure()`, so it does not verify the server certificate. Configure certificate verification before using it on an untrusted network. An HTTP success response alone does not prove that a reading was stored: the server also uses response bodies for device registration and measurement-point status.
